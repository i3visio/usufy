...
def getParser():
	...
	allPlatforms = config.getPlatforms(sites="all", mode="parser")
	platOptions = ["all"]
	...
	platOptions = sorted(platOptions)
	...
	groupPlatforms.add_argument('-p', '--platforms', metavar='<platform>', choices=platOptions, nargs='+', required=True, action='store', help='select the platforms where you want to perform the search amongst the following: ' + str(platOptions) + '. More than one option can be selected.')
	...
	groupAbout.add_argument('--version', action='version', version='%(prog)s 1.3.0', help='shows the version of the program and exists.')
